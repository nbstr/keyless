...
fn stub_with_session_verbs(
    ...
    let body = format!(
        "#!/bin/sh\n\
         ...
         \x20 key_fp=$(printf '%s' \"$PROTON_PASS_ENCRYPTION_KEY\" | shasum | cut -c1-12)\n\
         else\n\
         \x20 key_len=\"<unset>\"\n\
         \x20 key_fp=\"<unset>\"\n\
         fi\n\
         printf '%s %s current=%s key_provider=%s key_len=%s key_fp=%s\\n' \"$1\" \"$scoped\" \
         \"$now_current\" \"${{PROTON_PASS_KEY_PROVIDER-<unset>}}\" \"$key_len\" \"$key_fp\" >> '{verbs}'\n\
         case \"$1\" in\n\
         \x20 login)\n\
...
struct VerbLine {
    ...
    key_provider: String,
    key_len: String,
    /// A fingerprint of the key the child was handed — a hash prefix, never
    /// the value. A LENGTH cannot tell two keys apart, because every key this
    /// daemon generates is the same length: a run that generated one key,
    /// published a session under it, and then wrote a second over it reads
    /// identically to a correct one under a length oracle.
    key_fp: String,
}
...
fn parse_verbs(text: &str) -> Vec<VerbLine> {
    text.lines()
        .filter_map(|line| {
            ...
            let key_fp = fields
                .next()
                .and_then(|field| field.strip_prefix("key_fp="))
                .unwrap_or("<unset>")
                .to_owned();
            Some(VerbLine {
                verb,
...
fn a_daemon_whose_key_was_never_written_generates_one_and_logs_in_without_a_restart() {
    ...
    // ONE key, not merely one of the right length. Every key this generator
    // draws is 43 characters, so a length oracle passes a run that generated a
    // key, published a session under it, and then wrote a second one over it —
    // which is precisely the corruption a claim exists to prevent, and the
    // reason the stand-in records a fingerprint.
    let fingerprints: std::collections::BTreeSet<_> =
        lines.iter().map(|line| line.key_fp.clone()).collect();
    assert_eq!(
        fingerprints.len(),
        1,
        "the vendor was handed more than one local key across this run: {fingerprints:?}"
    );
