...
| `git show HEAD:.npmrc`, and its siblings `git cat-file -p`, `git grep <ref>`, `git diff <ref>`, `git log -p` | the bytes come out of the object store, so the path need not exist in the working tree and every filesystem question this pack asks answers about the wrong thing. **Deliberately not built, on measurement:** no repository on this machine tracks a file matching a protected pattern and none ever committed one, and across real traffic no invocation of any of these shapes named a protected path — while the shapes themselves are among the most common commands run. A pathspec predicate would add false-positive risk to ordinary git work to guard an empty population. A credential that is genuinely *tracked* is a larger incident than a read, and its remedy is rotation and a history rewrite |
